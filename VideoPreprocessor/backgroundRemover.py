...
class BGRemover:
    ...
    def checkIfCUDADevice(self):
        self.CUDA_enable = cuda.getCudaEnabledDeviceCount() > 0
    ...
    def startBGSubtract_CUDA(self):
        self.bgSubtractor = cuda.createBackgroundSubtractorMOG2()
        ...
        while True:
            ...
            gpu_fg_mask = self.bgSubtractor.apply(gpu_frame, learningRate=0, stream=self.stream)
            gpuBackground = self.bgSubtractor.getBackgroundImage(stream=self.stream)

            self.stream.waitForCompletion()

            fg_mask = gpu_fg_mask.download()
            if not gpuBackground.empty():
                background = gpuBackground.download()
    ...
    def BGSubtractor(self):
        if self.CUDA_enable:
            print("CUDA habilitado: usando GPU")
...
if __name__ == '__main__':
    ...
    path = 'Train_data/aic21-track4-train-data/aic21-track4-train-data/2.mp4'
    
    subtractor = BGRemover(path,'red.mp4', 'BG.mp4',GUI=True)
    subtractor.CUDA_enable = False
