...
class ImprovedWGANGPAnomalyDetector:
    ...
    def train(self, dataset: BalancedTrajectoryDataset, epochs: int = 50, batch_size: int = 64, 
        ...
        print(f"Starting improved WGAN-GP training:")
    ...
    def anomaly_score(self, trajectories: torch.Tensor) -> np.ndarray:
        """Improved anomaly score using multiple metrics"""
        self.generator.eval()
        ...
        with torch.no_grad():
            ...
            discriminator_score = -d_score  # Invert
            
            # 2. Simplified reconstruction score
            noise = torch.randn(batch_size, self.noise_dim).to(self.device)
            with torch.backends.cudnn.flags(enabled=False):
                fake_trajectories = self.generator(noise)
            
            # Find closest synthetic trajectory for each real one
            reconstruction_scores = []
            for i in range(batch_size):
                real_traj = trajectories[i]
                
                # Calculate distances to all synthetic trajectories
...
def main():
    ...
    print("IMPROVED ANOMALY DETECTION SYSTEM WITH WGAN-GP")
    print("=" * 60)
    ...
    try:
        ...
        if os.path.exists(ANOMALY_CSV):
            ...
            print("Anomaly file not found. Proceeding without labels.")
        ...
        dataset = BalancedTrajectoryDataset(
            ...
            balance_ratio=0.3  # 30% anomalies in validation
        )
        
        # Improved detector
        print("\nInitializing improved WGAN-GP...")
        ...
        if anomaly_labels:
            ...
            for percentile in [85, 90, 95, 97, 99]:
                ...
                print(f"   Percentile {percentile}: F1={results['f1_score']:.3f}, Precision={results['precision']:.3f}, Recall={results['recall']:.3f}")
            
            print(f"\nBest threshold: {best_threshold} percentile (F1-Score: {best_f1:.3f})")
            ...
            print("\nNo anomaly labels - detection test only...")
        ...
        print(f"\nImproved results saved in: {MODEL_DIR}")
